...
public class MiBandCommunicationService extends Service
{
    ...
    private static final WriteAction enableSensor = new WriteAction(MiBandConstants.UUID_CHARACTERISTIC_CONTROL_POINT, new byte[]{ (byte) 18, (byte) 1 });

    private static final WriteAction enableSteps = new WriteAction(MiBandConstants.UUID_CHARACTERISTIC_CONTROL_POINT, new byte[]{ (byte) 3, (byte) 1 });

    private static final WriteAction disableSensor = new WriteAction(MiBandConstants.UUID_CHARACTERISTIC_CONTROL_POINT, new byte[]{ (byte) 18, (byte) 0 });

    private static final WriteAction disableSteps = new WriteAction(MiBandConstants.UUID_CHARACTERISTIC_CONTROL_POINT, new byte[]{ (byte) 3, (byte) 0 });
    ...
    private BroadcastReceiver mColourReceiver = new BroadcastReceiver()
    {
        ...
        public void onReceive(Context context, Intent intent)
        {
            Log.i("Intent", "recieved colour broadcast");
    ...
    private void vibrate(final long duration)
    {
        ...
        {
            Toast.makeText(getApplicationContext(), "MiBand not paired", Toast.LENGTH_SHORT).show();
        }
    }
